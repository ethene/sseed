...
class TestPerformanceAndSecurity(unittest.TestCase):
    ...
    def tearDown(self) -> None:
        ...
        temp_patterns = ["*test_mnemonic*", "*offline_test*", "*offline_shards*", "*perf_test*", 
                        "_var_folders_*", "_tmp_*", "_Users_*"]
    ...
    def measure_performance(self, operation_func: callable, *args: Any, **kwargs: Any) -> tuple[float, Any]:
        ...
        print(f"Operation {operation_func.__name__}: {execution_time_ms:.2f}ms, Memory: {memory_used:.2f}MB")
    ...
    def test_38_mnemonic_generation_performance(self) -> None:
        ...
        print(f"Mnemonic Generation - Avg: {avg_time:.2f}ms, Min: {min_time:.2f}ms, Max: {max_time:.2f}ms")
        
        # Performance should be reasonable (well under 1 second, preferably under 100ms)
        self.assertLess(avg_time, 100, f"Average mnemonic generation time {avg_time:.2f}ms is too slow")
        self.assertLess(max_time, 200, f"Maximum mnemonic generation time {max_time:.2f}ms is too slow")
    ...
    def test_38_cli_operations_performance(self) -> None:
        ...
        try:
            ...
            start_time = time.perf_counter()
            result = subprocess.run([
                sys.executable, "-m", "sseed", "shard", 
                "-i", str(actual_test_file), "-g", "3-of-5", "-o", shards_file
            ], capture_output=True, text=True, cwd=Path(__file__).parent.parent)
    ...
    def test_39_memory_usage_limits(self) -> None:
        ...
        self.assertLess(max_memory, 100, f"Peak memory usage {max_memory:.2f}MB exceeds reasonable limit")
    ...
    def test_41_no_internet_calls(self) -> None:
        ...
        self.assertEqual(len(network_calls), 0, 
                        f"Unexpected network calls detected: {network_calls}")
    ...
    def test_41_offline_verification_cli(self) -> None:
        ...
        try:
            ...
            self.assertTrue(len(created_files) > 0, f"No output file was created. Expected pattern: *offline_test*. Found files: {list(Path('.').glob('*'))}")
            ...
            with open(actual_test_file, 'r', encoding='utf-8') as f:
                ...
                mnemonic_lines = [line for line in content.split('\n') 
            ...
            result = subprocess.run([
                sys.executable, "-m", "sseed", "shard", 
                "-i", str(actual_test_file), "-g", "2-of-3", "-o", shards_file
            ], capture_output=True, text=True, cwd=Path(__file__).parent.parent, timeout=10)
