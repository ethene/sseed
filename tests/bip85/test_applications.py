...
class TestBip85Applications:
    ...
    @patch("sseed.bip85.applications.entropy_to_mnemonic")
    def test_derive_bip39_mnemonic_entropy_error(
        self, mock_entropy_to_mnemonic, apps, test_master_seed
    ):
    ...
    def test_no_sensitive_data_in_exceptions(self, apps, test_master_seed):
        ...
        try:
            apps.derive_bip39_mnemonic(
                test_master_seed, 13, 0, "en"
            )  # Invalid word count
    ...
    def test_integration_with_existing_sseed_infrastructure(
        self, apps, test_master_seed
    ):
