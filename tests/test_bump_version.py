...
import importlib.util
spec = importlib.util.spec_from_file_location("bump_version", Path(__file__).parent.parent / "scripts" / "bump-version.py")
    ...
    def test_bump_version_types(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            for current, bump_type, expected in test_cases:
                ...
                assert result == expected, f"Bumping {current} {bump_type} should give {expected}, got {result}"
...
class TestGitIntegration:
    ...
    def test_git_commit_and_tag_real(self, mock_run):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            assert calls[1][0][0] == ["git", "commit", "-m", "chore: bump version to 1.0.1"]
