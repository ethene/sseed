...
from sseed.exceptions import MnemonicError, EntropyError
...
class TestBip39EdgeCases:
    ...
    def test_generate_mnemonic_entropy_failure(self):
        ...
        with patch("sseed.bip39.generate_entropy_bytes", side_effect=EntropyError("Entropy failed")):
    ...
    def test_generate_mnemonic_wrong_word_count(self):
        ...
        with patch("sseed.bip39.generate_entropy_bytes", return_value=b"x" * 32):
            ...
            with patch("sseed.bip39.Bip39MnemonicGenerator") as mock_gen:
                ...
                with pytest.raises(MnemonicError, match="Generated mnemonic has 3 words, expected 24"):
                    generate_mnemonic()
            ...
            with patch("sseed.bip39.Bip39MnemonicGenerator") as mock_gen:
                ...
                with patch("sseed.bip39.validate_mnemonic_words"):
                    with patch("sseed.bip39.Bip39MnemonicValidator") as mock_val:
                        ...
                        with pytest.raises(MnemonicError, match="Generated mnemonic failed BIP-39 checksum validation"):
                            generate_mnemonic()
    ...
    def test_generate_mnemonic_word_validation_failure(self):
        ...
        with patch("sseed.bip39.generate_entropy_bytes", return_value=b"x" * 32):
            ...
            with patch("sseed.bip39.Bip39MnemonicGenerator") as mock_gen:
                mock_gen.return_value.FromEntropy.return_value = mock_mnemonic
                with patch("sseed.bip39.validate_mnemonic_words", side_effect=Exception("Word validation failed")):
                    with pytest.raises(MnemonicError, match="Failed to generate BIP-39 mnemonic"):
                        generate_mnemonic()
    ...
    def test_parse_mnemonic_validation_failure(self):
        ...
        with patch("sseed.bip39.normalize_input", return_value="test mnemonic"):
            with patch("sseed.bip39.validate_mnemonic_words", side_effect=Exception("Validation failed")):
                with pytest.raises(MnemonicError, match="Failed to parse mnemonic"):
                    parse_mnemonic("test input")
    ...
    def test_get_mnemonic_entropy_exception_handling(self):
        ...
        with patch("sseed.bip39.validate_mnemonic", side_effect=Exception("Validation exception")):
            with pytest.raises(MnemonicError, match="Failed to extract entropy from mnemonic"):
