...
class TestCLIIntegration:
    ...
    def test_gen_command_to_file_success(self):
        ...
        mnemonic_lines = [line.strip() for line in content.split('\n') 
    ...
    def test_round_trip_integration(self):
        ...
        with open(mnemonic_file, 'r') as f:
            content = f.read()
        original_mnemonic = [line.strip() for line in content.split('\n') 
                           if line.strip() and not line.startswith('#')][0]
        
        # Step 2: Shard mnemonic to separate files
        exit_code, _, _ = self.run_sseed_command([
            "shard", "--separate", "-i", str(mnemonic_file), "-o", str(self.temp_dir / "shards.txt")
        ])
        assert exit_code == 0
    ...
    def test_shard_command_different_groups(self):
        ...
        exit_code, stdout, stderr = self.run_sseed_command([
            "shard", "-g", "2-of-3", "-i", str(mnemonic_file), "-o", str(self.temp_dir / "shards_2of3.txt")
        ])
        assert exit_code == 0
    ...
    def test_separate_files_feature(self):
        ...
        for i in range(1, 6):
            ...
            shard_lines = [line.strip() for line in content.split('\n') 
                          if line.strip() and not line.startswith('#')]
    ...
    def test_file_formats_compatibility(self):
        ...
        exit_code, stdout, stderr = self.run_sseed_command([
            "shard", "-i", str(mnemonic_file), "-o", str(self.temp_dir / "test_shards.txt")
        ])
        assert exit_code == 0
