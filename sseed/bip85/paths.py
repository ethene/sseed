...
def validate_bip85_parameters(
    ...
    logger.debug(
        ...
        application, length, index, strict
    )
...
def calculate_entropy_bytes_needed(application: int, length: int) -> int:
    ...
    if application == 39:  # BIP39
        ...
        return APPLICATION_ENTROPY_LENGTHS[39][length]
        ...
        logger.warning(
            "Unknown application %d, assuming 64-byte entropy requirement",
            application
        )
        return 64


def validate_derivation_index_range(index: int, max_index: Optional[int] = None) -> None:
...
def format_parameter_summary(application: int, length: int, index: int) -> Dict[str, Union[str, int]]:
