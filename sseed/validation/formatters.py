...
class ValidationFormatter:
    ...
    def format_text(
        ...
        lines.append("=" * 40)
        lines.append(f"{status_symbol} Overall Score: {colorize(str(overall_score), status_color)}/100")
    ...
    def format_summary(
        ...
        if "summary" in results:
            ...
            symbol = symbolize("pass" if success_rate >= 90 else "warning" if success_rate >= 70 else "fail")
    ...
    def _format_batch_text(
        ...
        status_color = "green" if success_rate >= 90 else "yellow" if success_rate >= 70 else "red"
        status_symbol = symbolize("pass" if success_rate >= 90 else "warning" if success_rate >= 70 else "fail")
        
        lines.append(f"{status_symbol} Overall Success Rate: {colorize(f'{success_rate:.1f}%', status_color)}")
        ...
        if statistics.get("quality_distribution"):
            ...
            for quality, count in quality_dist.items():
                if count > 0:
                    ...
                    lines.append(f"   {symbol} {quality.title()}: {colorize(str(count), color)}")
        ...
        if verbose and file_results and len(file_results) <= 20:
            ...
            for result in file_results:
                ...
                lines.append(f"   {symbol} {file_name}: {colorize(str(score), color)}/100")
...
def format_validation_output(
    ...
    if output_format == "json":
        ...
        return formatter.format_summary(results, use_colors=use_colors, use_symbols=use_symbols)
    else:  # text
        return formatter.format_text(results, verbose=verbose, use_colors=use_colors, use_symbols=use_symbols) 