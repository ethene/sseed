...
class CrossToolTester:
    ...
    def test_compatibility(self, mnemonic: str) -> CrossToolCompatibilityResult:
        ...
        for tool_name, tool_info in self.available_tools.items():
            try:
                ...
                tool_result = self._test_tool_compatibility(mnemonic, tool_name, tool_info)
                result.tool_results[tool_name] = tool_result
                result.tools_tested.append(tool_name)
                
                if tool_result.get("status") != "pass":
                    result.issues.append(f"{tool_name}: {tool_result.get('message', 'Unknown issue')}")
    ...
    def _test_shamir_cli_compatibility(
        ...
        try:
            ...
            # Combine results
            if round_trip_result["status"] == "pass" and entropy_result["status"] == "pass":
                return {
                    "status": "pass",
    ...
    def _test_slip39_round_trip(self, mnemonic: str) -> Dict[str, Any]:
        ...
        try:
            ...
            returncode, stdout, stderr = self._run_command("shamir recover", recovery_input)
    ...
    def _test_entropy_verification(self, mnemonic: str) -> Dict[str, Any]:
        ...
        try:
            ...
            master_secret_lines = [line for line in lines if "Using master secret:" in line]
            ...
            original_entropy_hex = master_secret_lines[0].split("Using master secret: ")[1].strip()
    ...
    def _calculate_compatibility_score(self, result: CrossToolCompatibilityResult) -> None:
        ...
        result.compatibility_score = int((total_score / max_score) * 100) if max_score > 0 else 0
    ...
    def _generate_recommendations(self, result: CrossToolCompatibilityResult) -> None:
        ...
        if result.compatibility_score < 80:
            recommendations.append("Mnemonic may not be fully compatible with external tools")
            
        if not result.tools_tested:
            recommendations.append("Install external tools for comprehensive compatibility testing")
            recommendations.append("Consider installing: pip install shamir-mnemonic[cli]")
            
        failed_tools = [
            tool for tool, result_data in result.tool_results.items()
            if result_data["status"] in ["fail", "error"]
        ]
        
        if failed_tools:
            recommendations.append(f"Review compatibility issues with: {', '.join(failed_tools)}")
            
        result.recommendations = recommendations
        
    def _run_command(self, cmd: str, input_text: Optional[str] = None) -> Tuple[int, str, str]:
