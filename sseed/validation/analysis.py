...
from ..bip39 import get_mnemonic_entropy, validate_mnemonic
from ..validation.crypto import validate_mnemonic_checksum
from ..validation.input import validate_mnemonic_words
from ..entropy.custom import validate_entropy_quality, EntropyQuality
from ..bip85.security import get_security_hardening
from ..languages import detect_mnemonic_language, SUPPORTED_LANGUAGES
from ..exceptions import ValidationError, CryptoError
...
class MnemonicAnalysisResult:
    ...
    def is_valid(self) -> bool:
        ...
        return (
            self.format_check.get("status") == "pass" and
            self.checksum_check.get("status") == "pass" and
...
class MnemonicAnalyzer:
        ...
        try:
            ...
            if detected_lang_info:
                ...
                if expected_language and detected_lang != expected_language:
                    ...
                    result.language_check["expected_name"] = expected_lang_info.name if expected_lang_info else "Unknown"
    ...
    def _analyze_entropy(self, mnemonic: str, result: MnemonicAnalysisResult) -> None:
        ...
        try:
            ...
            if entropy_quality.warnings:
                result.warnings.extend([f"Entropy: {w}" for w in entropy_quality.warnings])
    ...
    def _analyze_security(
        ...
        try:
            ...
            result.security_analysis = {
                ...
                "message": "Meets security requirements" if security_valid else "Below security threshold",
                "details": {
                    "has_weak_patterns": self.security_hardening._has_weak_patterns(entropy_bytes),
                    "passes_randomness_test": self.security_hardening._passes_chi_square_test(entropy_bytes),
                }
            }
            
            if not security_valid:
                result.warnings.append("Entropy does not meet security requirements")
                result.security_notes.append("Consider regenerating with system entropy")
    ...
    def _analyze_weak_patterns(self, mnemonic: str, result: MnemonicAnalysisResult) -> None:
        ...
        try:
            ...
            result.weak_patterns = {
                ...
                "message": "; ".join(issues) if issues else "No obvious weak patterns detected",
                "details": {
    ...
    def _generate_recommendations(self, result: MnemonicAnalysisResult) -> None:
        ...
        if not result.security_analysis.get("meets_security_threshold", True):
            recommendations.append("Use stronger entropy source for security-critical applications")
    ...
    def _analyze_entropy_patterns(self, entropy_bytes: bytes) -> Dict[str, Any]:
        ...
        return {
            ...
            "byte_distribution": "uniform" if len(set(entropy_bytes)) > len(entropy_bytes) / 2 else "skewed",
        }
